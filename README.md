...
### スタートダイアログ（`--classic-notenest` 使用時）
- `NoteNest.exe --classic-notenest` で従来の NoteNest 単体版を起動すると「NoteNest をはじめる」ダイアログが表示される
- 「新規プロジェクトを開始する」または最近使ったファイル（最大 5 件）から次の操作をすぐに選択できる
- `--classic-notenest sample.notenest` のようにファイルを指定するとダイアログをスキップして直接ファイルを開く
- v1.11.0 以降、`NoteNest.exe`（フラグなし）は NestSuite が起動するためスタートダイアログは表示されない
