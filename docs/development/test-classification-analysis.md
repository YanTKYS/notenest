...
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SchemaVersionConstant_Is_1_4_1` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_SchemaVersionConstant_Is_1_1_4` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `ChatNestFormatSchemaRegressionTests` | `ChatNest_FileVersionConstant_Is_0_4_1` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `ChatNestFormatSchemaRegressionTests` | `ChatNest_FileExtensionConstant_Is_chatnest` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_FileExtensionConstant_Is_ideanest` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `TempNestFormatSchemaRegressionTests` | `TempNest_DefaultJsonVersion_Is_1` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SerializedJson_ContainsProjectNameKey` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SerializedJson_ContainsNotebooksKey` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SerializedJson_ContainsTasksKey` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SerializedJson_ContainsSettingsKey` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SavedJson_IsValidJson` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SaveLoad_PreservesSchemaVersion` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNest_SaveLoad_RoundTrip_PreservesNotebookAndNoteStructure` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_SerializedJson_ContainsVersionField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_SerializedJson_ContainsIdeasField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_SerializedJson_ContainsSettingsField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_SavedJson_IsValidJson` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `IdeaNestFormatSchemaRegressionTests` | `IdeaNest_SaveLoad_PreservesVersion` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `ChatNestFormatSchemaRegressionTests` | `ChatNest_SerializedJson_ContainsVersionField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `ChatNestFormatSchemaRegressionTests` | `ChatNest_SerializedJson_ContainsMessagesField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `ChatNestFormatSchemaRegressionTests` | `ChatNest_SavedJson_IsValidJson` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `ChatNestFormatSchemaRegressionTests` | `ChatNest_SaveLoad_PreservesMessages` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `TempNestFormatSchemaRegressionTests` | `TempNest_StoreData_DefaultVersionIs1` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `TempNestFormatSchemaRegressionTests` | `TempNest_StoreData_HasSlotsArray` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `SessionFormatSchemaRegressionTests` | `Session_DefaultState_HasEmptyFilePathsAndNullActivePath` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `SessionFormatSchemaRegressionTests` | `Session_SerializedJson_ContainsFilePathsField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `SessionFormatSchemaRegressionTests` | `Session_SerializedJson_ContainsActiveFilePathField` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `SessionFormatSchemaRegressionTests` | `Session_RoundTrip_PreservesFilePathsAndActivePath` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
| `NoteNestFormatSchemaRegressionTests` | `NoteNestSchemaVersion_Remains_1_4_1` | ドキュメント / ルール固定テスト | docs / version / schema rule |  |  |
...
| ファイル | 用途 |
|----------|------|
| `NoteNestFormatSchemaRegressionTests` / `IdeaNestFormatSchemaRegressionTests` / `ChatNestFormatSchemaRegressionTests` / `TempNestFormatSchemaRegressionTests` / `SessionFormatSchemaRegressionTests` | 保存形式回帰（schema version が実ファイルに書き込まれること）— TD-48 にて `FormatSchemaRegressionTests` を対象別 5 ファイルに分割 |
...
## 8. v2.10.16 対応記録（TD-30 追補）

### 実施内容

v2.10.16 TD-30 にて、テストクラス乱立抑制・集約方針を development guidelines に明文化した。

#### 追加した方針（`nestsuite-development-guidelines.md` §7 への追記）

| 方針 | 内容 |
|------|------|
| 新規テストクラスの命名原則 | 対象クラス名 + Tests、または対象責務名 + Tests |
| 既存テストクラス優先 | 新課題でテストを追加する場合、まず既存クラスへ追加できないか確認する |
| 課題番号ベース命名禁止 | `CH8CH14Tests`、`TD25Tests`、`V2103Tests` のような命名は避ける |
| backlog ID の追跡方法 | テストクラス名ではなく、メソッドコメントまたは `Trait("Backlog", "...")` に残す |
| 既存クラスの段階整理 | 既存の課題番号ベーステストクラスは一括変更せず、触るタイミングで段階的に集約する |

#### このバージョンで行わなかったこと

- 既存テストクラスの大規模リネーム・移動・統合
- テストメソッドの削除・移動
- テストロジックの変更

### 次段階の整理方針（参考）

一次分析（TD-28）で特定した課題番号ベーステストクラスは、今後の段階整理で以下の方向性で集約する。
一括変更ではなく、機能追加・修正のタイミングで段階的に対応する。

| 現在のクラス名（例） | 集約候補 |
|---------------------|---------|
| `ChatNestCH8CH14Tests` | `ChatNestFileServiceTests` や `ChatNestWorkspaceTests` 等 |
| `LightImprovementsV2103Tests` | 対象クラス単位のテストクラスへ分散 |
| `SessionNestTD25Tests` | `SessionTabMapperTests` や `WorkspaceSessionManagerTests` 等 |
...
## 10. v2.10.18 対応記録（TD-32 テストクラス集約）

### 実施内容

v2.10.18 TD-32 にて、TD-28 の一次分析および TD-30 の「テストクラス乱立抑制・集約方針」に基づき、課題番号・バージョンベースの既存テストクラスを既存の受け皿へ集約した。

単なる別名変更ではなく、対象クラス単位・責務単位・*Nest 単位の既存テストクラスへテストメソッドを移動した。テストロジックの意味変更は行っていない。

### 旧テストクラスと集約先の対応表

| 旧テストクラス | 主な移動先 | 関連ID | 備考 |
|---------------|------------|--------|------|
| `SessionNestTD25Tests` | `SessionTabMapperTests` | TD-25 | session.json 形状、TempNest session 対象外、detached 非保存、復元ターゲット生成を SessionTabMapper の責務境界テストへ集約。 |
| `GuardNestTD26Tests` | `AtomicFileWriterTests` | TD-26 | AtomicFileWriter / CloseConfirmationService / FileErrorMessages / ErrorLogService の GuardNest 境界確認を、既存の保存安全性・GuardNest 責務確認の受け皿へ集約。ただし `AtomicFileWriterTests` の責務が広がるため、今後は `GuardNestTests` / `FileSafetyTests` 等の責務単位受け皿、または対象クラス単位への再分配を再判断する。 |
| `LightImprovementsV2103Tests` | `TempNestTests` | TN-2 / L14 / L15 / CH-13 / v2.10.3 | TempNest スロット clear 回帰確認を TempNest 単位の既存テストクラスへ集約し、関連 backlog / release notes 確認も同じ受け皿へ移動。 |
| `ShellUxTests` | `NestSuiteShellTests` | SH-6 / SH-9 / SH-13 / SH-18 | Shell UX の静的確認を Shell 境界・構造確認の既存受け皿へ集約。 |
| `UiSmokeTD23Tests` | `NestSuiteShellTests` | TD-23 | UI smoke program の構造確認と TD-23 docs 確認を Shell / UI smoke 境界の既存受け皿へ集約。 |
| `V140RegressionTests` | `FormatSchemaRegressionTests` | v1.4.0 | NoteNest 保存・読込・バックアップ・選択/設定・リンク回帰を保存形式 / schema 回帰の受け皿へ集約。 |
| `V141FeatureTests` | `FormatSchemaRegressionTests` | v1.4.1 | Note timestamp / export / autosave / project info 確認を保存形式 / schema 回帰の受け皿へ集約。 |
| `V146RegressionTests` | `FormatSchemaRegressionTests` | v1.4.6 | NoteNest 保存・読込・autosave・recent files・timestamp・export 回帰を保存形式 / schema 回帰の受け皿へ集約。 |

参考として、v2.10.17 TD-31 で `ChatNestCH8CH14Tests` / `ChatNestCH9ExportTests` / `ChatNestCH13DragReorderTests` はすでに `ChatNestWorkspaceViewModelTests` / `ChatNestExportFormatterTests` へ集約済みである。

### テストクラス数の増減

- v2.10.18 変更前テストクラス数: 94
- v2.10.18 変更後テストクラス数: 86
- v2.10.18 増減: 8 クラス削減
- TD-28 起点の累計: 96 → 86
- テストクラス数は増やしていない。
...
`GuardNestTD26Tests` は乱立抑制を優先して既存の `AtomicFileWriterTests` へ集約したが、CloseConfirmationService / FileErrorMessages / ErrorLogService / 方針文書確認も含むため、長期的には `GuardNestTests` や `FileSafetyTests` のような責務単位の受け皿、または対象クラスごとの既存テストクラスへの再分配を検討する。

### backlog ID の追跡方法

backlog ID はテストクラス名ではなく、移動先クラス内のコメント、テストメソッド名、または本対応表で追跡する。Trait の追加は既存方針に合わせて必須化せず、コメント・メソッド名で追跡する。
