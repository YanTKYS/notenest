...
| L20 | `.notenest` 読込失敗メッセージへの `.bak` 復元の言及 | 現状、破損または未対応形式の `.notenest` を開こうとすると「ファイル形式を読み取れません。ファイルが破損している可能性があります。」というエラーダイアログのみが表示され、`.bak` バックアップファイルが存在する可能性には触れない。エラーダイアログの文言に、直前の保存で作成された `.bak` ファイルから復元できる可能性がある旨を短く追記し、失敗した瞬間に次の行動を示す。L8（ヘルプメニューからの `.bak` 復元ガイド導線）とは別軸で、読込失敗という具体的な発生タイミングに合わせた案内を補う位置づけ | B |
...
| M17 | 検索結果のマッチ箇所ハイライト表示 | 現状、全ノート検索結果一覧はプレーンテキストの前後文脈のみを表示し、検索語に一致した箇所がどこかは目視で探す必要がある。結果一覧の該当部分を太字またはハイライト色で強調表示し、目的の一致箇所を素早く見つけられるようにする | B |
...
| ID-14 | カラーフィルタチップへのカード枚数表示 | 現状、色フィルタは色名のみをツールチップ表示し、その色が何枚のカードに使われているかは分からない。カード色は利用者が自由に割り当てる方式のため固定の意味付けはないが、フィルタチップに枚数を併記することで、利用者が自分で決めた色分けを思い出しやすくする。保存形式変更なし | B |
| ID-15 | 新規カード作成後の位置フィードバック | 現状、ダイアログからカードを新規作成すると（`AddIdea` → `CommitAdd`）、ダイアログが閉じてカード一覧が再構築されるだけで、追加されたカードがどれかを示す選択・ハイライト・スクロールは行われない。並べ替え順によっては一覧のどこに追加されたか探す必要がある。作成直後のカードを選択状態にし、必要であれば表示領域へスクロールすることで、作成結果をその場で確認できるようにする | B |
...
TD-1〜TD-52、TD-54、TD-55 は完了済み（欠番）。詳細は `docs/release-notes.md` 参照。
