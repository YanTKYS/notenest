...
| N21 | IdeaNest `.ideanest` 保存・読込 | NestSuite の IdeaNest タブで `.ideanest` ファイルの保存・読込を実装する。ファイルメニューの新規・開く・保存・名前を付けて保存を有効化する。v1.8.2 で方針整理・`IdeaNestFileService` スケルトン・モデルの `[JsonPropertyName]` 付与済み。v1.8.3 で UI ワイヤリングを実装予定 | B |
