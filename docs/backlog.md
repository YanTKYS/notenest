...
| ~~M3~~ | ~~右ペインのリンク管理タブ（リンク切れ・バックリンク）~~ | **完了（v2.6.9）** | — |
...
| TD-4 | 例外処理の種別分け・診断性向上 | ✅ v2.7.5 で対応済み。`FileErrorMessages`（`NestSuite.Services`）を新設し `FileNotFoundException` / `UnauthorizedAccessException` / `JsonException` / `IOException` 等で分岐。NestSuiteShellWindow.FileOperations.cs の全 `catch (Exception ex)` ブロック（9 箇所）でユーザー向けメッセージを種別ごとに出し分けるよう更新。TempNest・SessionState の silent-failure パスにも Error ログを追加 | B |
