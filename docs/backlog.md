...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| N5 | NestSuite 最小 AppShell 骨格 | `NoteNestWorkspaceView` をホストする WPF Window の最小構成を作成する。メニュー・ステータスバー・ウィンドウ設定なしの骨格のみ。`IWorkspaceDialogHost` の NestSuite 側実装も骨格として追加。NoteNest 単体版 `MainWindow` は引き続き維持する | B |
