...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-3 | 3 Workspace のファイル操作重複の汎用化 | ✅ v2.7.15 で TD-3-4 まで完了。Load/Save/SaveAs（TD-3-1）・新規作成/タブクローズ確認（TD-3-2）・セッション復元/タブ同期（TD-3-3）・保存成功後の tab/session 更新統一（TD-3-4）を実施済み。今後の大きな整理は TD-6 系で扱う | C |
