...
| ~~TD-32~~ | ~~テストクラス集約~~ | v2.10.18 完了。`SessionNestTD25Tests` / `GuardNestTD26Tests` / `LightImprovementsV2103Tests` / `ShellUxTests` / `UiSmokeTD23Tests` / `V140RegressionTests` / `V141FeatureTests` / `V146RegressionTests` を対象クラス単位・責務単位・*Nest 単位の既存受け皿へ集約した。テストクラス数は増やさず 94 → 86 に削減（前回修正後の 91 から追加で 5 クラス削減。TD-28 起点の累計では 96 → 86）。テストロジックの意味変更なし。 | — |
