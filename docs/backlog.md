...
| SH-21 | 複数モニタ向け Workspace 別ウィンドウ表示 | **v2.9.0 試作 → v2.9.1 安定化 → v2.9.2 保存・終了確認整合で完了。** タブ右クリック→「別ウィンドウで表示」で `DetachedWorkspaceWindow` を開く。同 VM 共有・Ctrl+S・ウィンドウ閉じると自動再統合。Shell プレースホルダーの「このタブへ戻す」ボタン・コンテキストメニュー切り替え。別ウィンドウ SaveAs ダイアログの Owner 修正・重複チェックの tabId 基準修正。TempNest / IdeaNest / ChatNest は未対応。 | B |
