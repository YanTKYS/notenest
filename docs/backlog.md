...
| SH-21 | 複数モニタ向け Workspace 別ウィンドウ表示 | **v2.9.0 NoteNest 試作 → v2.9.1 安定化 → v2.9.2 保存・終了確認整合 → v2.9.3 IdeaNest 対応で完了。** タブ右クリック→「別ウィンドウで表示」で `DetachedWorkspaceWindow` を開く。同 VM 共有・Ctrl+S・ウィンドウ閉じると自動再統合。Shell プレースホルダーの「このタブへ戻す」ボタン・コンテキストメニュー切り替え。`DetachedWorkspaceWindow` を Workspace 非依存に改修（`UIElement` コンストラクタ）。NoteNest / IdeaNest 両対応。TempNest / ChatNest は未対応。 | B |
...
- **TD-13 / TD-15 / TD-16 は v2.7.16〜v2.7.18 で完了済み。TD-14 は v2.8.6・TD-17 は v2.8.7・TD-11 は v2.8.8・TD-18 は v2.8.9・TD-19 は v2.8.10 で完了済み。** 技術的負債の未完了項目は現時点でなし。
