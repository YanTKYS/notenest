...
| SH-5 | タブ末尾の新規タブ「＋」ボタン | タブストリップ末尾に「＋」を置き、クリックでツール選択（NoteNest / IdeaNest / ChatNest）して新規タブを作成できるようにする。ヘッダーのランチャーまで視線移動せずに済む | A |
| SH-6 | タブ過多時のオーバーフロー一覧 | タブが横幅に収まらない場合に、ドロップダウンで全タブを一覧表示し選択ジャンプできるようにする。横スクロールだけに頼らない導線を用意する | B |
| SH-7 | ツール種別のタブ視覚識別 | タブに NoteNest / IdeaNest / ChatNest を示すアイコンまたは色アクセントを付け、種別を一目で区別できるようにする。ファイル名だけでは種別が分かりにくい問題を解消する | B |
...
| CH-3 | 最新発言への自動スクロール・最下部移動ボタン | 発言追加時に最新発言へ自動スクロールし、スクロールを遡っているときは「最新へ」ボタンを表示する。会話が伸びた際の見失いを防ぐ | A |
| CH-4 | 発言削除確認の Workspace 内ダイアログ化 | 発言削除確認に直接使用している `MessageBox`（design-decisions §35 の暫定許容）を、他 Workspace と同様のアプリ内ダイアログに置き換え、見た目と操作感を統一する | B |
| CH-5 | 会話内検索 | 表示中の会話を対象に文字列検索し、一致発言へジャンプ・ハイライトできるようにする。長い会話から目的の発言を探しやすくする | B |
| CH-6 | 既存発言のインライン編集 | 確定済みの発言をその場で編集できるようにする。誤字修正のために削除・再入力する手間をなくす。保存形式の互換を保てる範囲で検討する | B |
