...
| L9 | 最近使ったファイルのクリア | 最近使ったファイル一覧を手動でクリアできるようにする。共有端末や検証時に不要な履歴を残したくない場合に有用 | B |
| L10 | 保守性改善：MainViewModel の責務棚卸し | MainViewModel 内の責務を、ノート管理、タスク管理、エディタ状態、保存状態、リンク関連などに分類し、将来の分割単位を明確にする。実装変更は最小限 | A |
| L11 | 保守性改善：MainWindow.xaml.cs のイベント処理棚卸し | ダイアログ生成、ドラッグ＆ドロップ、右クリックメニュー、ショートカットキー処理などを分類し、切り出し候補を整理する | A |
...
| M10 | プロジェクト情報ダイアログ | プロジェクト名、ファイルパス、ノートブック数、ノート数、タスク数、マーカー数、最終保存日時などを確認できるダイアログを追加する | B |
| M12 | 保守性改善：ダイアログ生成処理の整理 | InputDialog、NotePickerDialog、TutorialWindow などの生成処理を共通化、または軽量な DialogService 化する | B |
| M13 | 保守性改善：MainViewModel の段階的分割 | ノート管理、タスク管理、マーカー、エディタ状態などを小さな Service または ViewModel へ段階的に分離する | B |
