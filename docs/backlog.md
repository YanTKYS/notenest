...
| SH-26 | ツールチップへのショートカットキー表記統一 | ショートカットが割り当てられている全 UI 要素のツールチップに `(Ctrl+X)` 形式で統一記載する。現在は一部のみ（例: 「保存 (Ctrl+S)」は表示、NoteNest エディタツールバーの Bold/Italic/マーカー挿入ボタン等は未表示）。SH-19（ショートカット一覧ダイアログ）とは補完関係 | A |
| SH-27 | コンテキストメニューへの InputGestureText 追加 | Shell のトップレベルメニューには `InputGestureText` でショートカットが表示されるが、同じ操作をもつコンテキストメニュー項目には未設定。NoteNest エディタの右クリックメニュー（コピー・貼り付け等）や IdeaNest カードのコンテキストメニュー等に追加し、ショートカットの発見性を高める | B |
...
| L17 | 右ペイン開閉ボタンのツールチップを開閉状態に連動 | v2.12.7 で統一された開閉ボタンのツールチップが常に「右ペインを表示」のまま。閉じている状態では「右ペインを表示」、開いている状態では「右ペインを折り畳む」に切り替える。`CollapseRightPane` / `ExpandRightPane` 内で `RightPaneExpandButton.ToolTip` を設定する | A |
...
| TD-50 | 確認ダイアログの文言パターン統一 | ノート削除・タスク削除・カード削除・発言削除など破壊的操作の確認ダイアログで、文言構造（「〇〇を削除しますか？」「この操作は元に戻せません。」等）が微妙に異なる。統一パターンを定め、操作対象・不可逆性・影響範囲を一貫したフォーマットで伝えるようにする。ダイアログ表示を担う `IWorkspaceDialogHost` 実装側の整理も含む | B |
