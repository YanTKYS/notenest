...
| SH-21 | 複数モニタ向け Workspace 別ウィンドウ表示 | **v2.9.0 で NoteNest を対象とした最小試作を実施。** タブ右クリック→「別ウィンドウで表示」で `DetachedWorkspaceWindow` を開く。同 VM 共有・Ctrl+S・ウィンドウ閉じると自動再統合。TempNest / IdeaNest / ChatNest は未対応。詳細設計: `architecture/workspace-detached-window.md` 参照 | B |
