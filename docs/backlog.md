...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| N4 | 将来的なWorkspace切り出し検討 | NestSuite本体の方針が固まった段階で、実際の切り出し設計と段階的移行計画を立案する。DataContext は案A（MainViewModel継続）/案B（NoteNestWorkspaceViewModel新設）/案C（MainViewModel分割）から選択する | C |
...
※ N3「NoteNestWorkspaceView 構想の設計」は v1.5.3 で設計メモを追加して完了。
  実切り出しは N4 で NestSuite 本体設計と合わせて進める（design-decisions.md §26・nestsuite-preparation.md 参照）。
