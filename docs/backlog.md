...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-3 | 3 Workspace のファイル操作重複の汎用化 | **第一段階（TD-3-1）完了（v2.7.6）。** Load / Save / SaveAs 周辺の共通化済み。残課題: `ConfirmAndReset*Nest` / `Sync*TabForViewModel` の共通化、保存成功後の tab/session 更新統一、セッション管理との統合整理 | B |
