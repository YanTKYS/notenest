...
| SH-16 | 起動時に前回アクティブ Workspace タブを即時表示（ちらつき抑制） | セッション復元でアクティブタブが決まるまでの間、Temp タブが一瞬表示される場合がある。セッション状態を先読みしてコンストラクタ内で先にアクティブタブを確定させ、ウィンドウ表示前のちらつきを完全に除去する | B |
...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TN-1 | スロットのプレースホルダーテキスト | 空のタイトル・本文欄に薄い案内テキスト（例：「タイトル」「メモを入力…」）を表示し、初見ユーザーに使い方を伝える。WPF 標準 TextBox には `PlaceholderText` がないため、`VisualBrush` + `IsEnabled` Trigger または `TextBox` を重ねた TextBlock で実装する | B |
...
| L11 | ノートの複製 | 左ペインのノートコンテキストメニューに「複製」を追加し、同名 + "（コピー）" の新ノートを末尾に作成する。タスク・マーカーも含め本文ごとコピーする。スキーマ変更不要 | A |
...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-2 | `NestSuiteShellWindow.xaml.cs` の partial 分割 | v2.6.0 時点で約 1800 行・10 責務超に膨張している。動作を変えずに `NestSuiteShellWindow.TabManagement.cs` / `.FileOperations.cs` / `.SessionPersistence.cs` / `.UiSync.cs` などの partial へ機械的に分割し、可読性・変更容易性を上げる。`MainWindow` / `MainViewModel` の partial 分割（design-decisions §22・§16）と同じ整理方針 | B |
...
| TD-4 | 例外処理の種別分け・診断性向上 | ファイル読込・保存の `catch (Exception ex)` が約 8 箇所あり、いずれも `ex.Message` をそのまま表示するため原因（不存在／アクセス権／形式不正）を区別できない。`FileNotFoundException` / `UnauthorizedAccessException` / `JsonException` 等で分岐し、ユーザー向けに具体的な対処メッセージを出す。あわせて TD-5 と連動した軽量ログ出力を検討 | B |
...
| TD-9 | `TempNestWorkspaceViewModel` の IDisposable 実装 | `DispatcherTimer` を保持しているが `IDisposable` を実装しておらず、`OnClosed` の `IDisposable` ループ（TD-1）から `Dispose` が呼ばれない。`_saveTimer.Stop()` と各スロットの `Changed` イベント購読解除を `Dispose` に集約し、`MainViewModel` / `ChatNestWorkspaceViewModel` / `IdeaNestWorkspaceViewModel` と統一する | A |
| TD-10 | テストカバレッジ拡充（Tab・Session・TempNest 中心） | `NestSuiteDocumentTabTests` に `CanClose`・`TempNest` 固有ケースが少ない。`CreateTempTab()` のプロパティ検証・`TryGetKind` が Temp を除外することの検証・`TempNestStoreService` の JSON ラウンドトリップテストを追加し、回帰検出を強化する | B |
