...
| SH-2 | タブのコンテキストメニュー | タブ右クリックで「閉じる／他のタブを閉じる／右側のタブを閉じる／すべて閉じる」を選べるようにする。タブが増えた際の整理操作を高速化する | A |
| SH-3 | タブの閉じるボタン・中クリックで閉じる | 各タブに「✕」ボタンを表示し、ホイールクリック（中クリック）でも閉じられるようにする。現状はメニュー経由のみのため操作数を減らす | A |
| SH-4 | タブ切替キーボードショートカット | `Ctrl+Tab` / `Ctrl+Shift+Tab` で前後タブ移動、`Ctrl+1`〜`Ctrl+9` で番号指定タブへ移動できるようにする。マウスを使わないタブ移動を可能にする | A |
...
| SH-11 | 無効状態ボタンのカーソル・視覚フィードバック統一 | `FlatButton` 系スタイルは `IsEnabled=False` 時に Opacity を下げるのみで、`Cursor="Hand"` が残りクリック可能に見える。無効時に `Cursor="Arrow"` へ切り替え、配色も明確に落とす Trigger を共通スタイルへ追加する | A |
| SH-12 | メニュー・ダイアログのアクセスキー（ニーモニック）完全化 | トップメニューにはアクセスキーがあるが、NoteNest のコンテキストメニュー各項目・FindReplaceDialog のボタン（次を検索／置換／すべて置換）・ChatNest 発言者ラジオボタン等に `_`（ニーモニック）が未設定。キーボードだけで操作できる範囲を広げる | A |
...
| ID-3 | 詳細ウィンドウのサイズ・位置記憶 | カード詳細（`PreviewIdeaWindow`）のサイズ・位置を記憶し、次回開いたときに復元する。v2.1.x で常時編集ウィンドウ化したため、編集体験の継続性向上に効く | A |
...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-1 | ChatNest / IdeaNest ViewModel のライフサイクル整理（Dispose 実装） | `ChatNestWorkspaceViewModel` / `IdeaNestWorkspaceViewModel` は `DispatcherTimer`（ステータス消去用）や `CollectionChanged` / `PropertyChanged` 購読を持つが `Dispose` を持たず、タブを閉じてもタイマー・購読が解除されない。`MainViewModel.Dispose()`（タイマー停止＋イベント解除、`ConfirmAndResetNoteNest` から呼出）と同じ前例に揃え、両 ViewModel に `Dispose` を実装してタブクローズ時に呼ぶ。長時間運用でのタブ生成・破棄を繰り返した際の GC リーク・幽霊タイマーを防ぐ | A |
