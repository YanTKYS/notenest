...
| L9 | 右ペイン展開ボタンとスクロールバーの重なり回避 | 右ペイン折り畳み時の展開ボタン（「»」）がエディタの縦スクロールバーと重なりクリックしづらい場合がある。ボタン配置・列幅・Margin を見直し、スクロールバー幅を考慮して常に押せるようにする | B |（v2.7.10 完了 → `docs/release-notes.md` 参照）|
...
| L12 | エディタのフォントサイズ変更 | 設定メニューにフォントサイズ（11 / 13 / 15 / 17pt 程度のステップ）を追加し、`EditorBox` の `FontSize` を `DynamicResource` で切り替える。`UiSettings` に保存・次回起動時に復元する | B |（v2.7.10 完了 → `docs/release-notes.md` 参照）|
...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-3 | 3 Workspace のファイル操作重複の汎用化 | **第三段階（TD-3-3）完了（v2.7.8）。** Load/Save/SaveAs（TD-3-1）・新規作成・タブクローズ確認（TD-3-2）・セッション復元・タブ同期（TD-3-3）の共通化済み。残課題: 保存成功後の tab/session 更新統一（`UpdateChatNestTabPath` 等）、セッション管理との統合整理 | B |
