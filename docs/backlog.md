...
| SH-21 | 複数モニタ向け Workspace 別ウィンドウ表示 | 選択した Workspace（NoteNest 等）を同一プロセス内の追加 Window として分離表示できるようにする。複数モニタでの作業効率向上を目的とする。詳細設計: `architecture/workspace-detached-window.md` 参照 | B |
