...
| SH-21 | 複数モニタ向け Workspace 別ウィンドウ表示 | **v2.9.0 NoteNest 試作 → v2.9.1 安定化 → v2.9.2 保存・終了確認整合 → v2.9.3 IdeaNest 対応 → v2.9.4 ChatNest 対応で完了。** タブ右クリック→「別ウィンドウで表示」で `DetachedWorkspaceWindow` を開く。同 VM 共有・Ctrl+S・ウィンドウ閉じると自動再統合。Shell プレースホルダーの「このタブへ戻す」ボタン・コンテキストメニュー切り替え。NoteNest / IdeaNest / ChatNest 対応。TempNest は対象外。 | B |
