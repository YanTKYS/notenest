...
| ~~TD-29~~ | ~~NoteNest schema テスト集約・重複 schema テスト抑制~~ | v2.10.15 完了。各機能テストクラスに散在していた `NoteNestSchemaVersion_Remains_1_4_1` メソッド（20 ファイル）を削除し、`ApplicationVersionTests.cs` に集約した。`NoteNestSchemaVersion_IsNotTested_InOtherTestClasses` テストを追加して再散在を自動検出できるようにした。開発ガイドライン §7 に集約ルールを追記した。 | — |
| ~~TD-30~~ | ~~テストクラス乱立抑制・集約方針の明文化~~ | v2.10.16 完了。テストクラスを課題番号・バージョン単位で乱立させない方針を development guidelines §7 に追記した。新規テストは既存テストクラスへの追加を優先する方針、backlog ID はコメントまたは Trait で追跡する方針を明記した。既存テストクラスの大規模リネーム・移動・統合は未実施。 | — |
| ~~TD-31~~ | ~~ChatNest 系テストクラス集約 第一段階~~ | v2.10.17 完了。`ChatNestCH8CH14Tests`・`ChatNestCH9ExportTests`・`ChatNestCH13DragReorderTests` の 3 クラスを削除し、`ChatNestWorkspaceViewModelTests`（CH-8/9/13 テスト追加）および新規 `ChatNestExportFormatterTests`（CH-9 Markdown / CH-14 PlainText テスト収録）に集約した。テストクラス数 96 → 94。 | — |
| ~~TD-32~~ | ~~テストクラス集約~~ | v2.10.18 完了。`SessionNestTD25Tests` / `GuardNestTD26Tests` / `LightImprovementsV2103Tests` / `ShellUxTests` / `UiSmokeTD23Tests` / `V140RegressionTests` / `V141FeatureTests` / `V146RegressionTests` を対象クラス単位・責務単位・*Nest 単位の既存受け皿へ集約した。テストクラス数は増やさず 94 → 86 に削減（前回修正後の 91 から追加で 5 クラス削減。TD-28 起点の累計では 96 → 86）。テストロジックの意味変更なし。最終再分析で4分類へ整理完了。 | — |
...
| ~~TD-29~~ | ~~NoteNest schema テスト集約~~ | v2.10.15 完了 |
| ~~TD-30~~ | ~~テストクラス乱立抑制・集約方針~~ | v2.10.16 完了 |
| ~~TD-31~~ | ~~ChatNest 系テストクラス集約 第一段階~~ | v2.10.17 完了 |
| ~~TD-32~~ | ~~テストクラス集約~~ | v2.10.18 完了 |
