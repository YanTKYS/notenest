...
H0 系列はすべて完了。今後の実装は下記「高難易度」セクションの EH-1 → H2a → H1a の順で進める（`docs/design/notenest-editor-h0-reassessment.md` §6 参照）。
...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| EH-1 | NoteEditorHost 最小実装 | `LineNumberBox` と `EditorBox` を内包する `NoteEditorHost` UserControl を追加する。UI・動作に変化はなく、H1a / H2a 着手の前提となる | B |
| H2a | 行番号ガターの現在行強調 | 現行 `LineNumberBox`（TextBox）を ItemsControl 方式に変更し、現在編集中の行番号を視覚的に強調する。エディタ本文内の背景ハイライトは行わない。EH-1 完了後に着手 | C |
| H1a | 簡易ノートリンク補完 | `[[` 入力時にノートタイトルの候補 Popup を表示し、選択で `[[ノート名]]` を挿入する。厳密なキャレット追従ではなくキャレット行付近への表示から始める。EH-1 完了後に着手 | C |
