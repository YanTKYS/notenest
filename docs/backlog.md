...
|-----------|------|--------|
| M2 | マーカーからタスクを作成 | A |
| SH-19 | ヘルプメニューにキーボードショートカット一覧 | B |
| M15 | マーカー / タスクの一括コピー | B |
| M12 | ノートのスター（お気に入り）機能 | B |
| M13 | 左ペインのノート手動並び替え | B |
| M14 | ノートの並び替え（作成日/更新日/タイトル順） | B |
| L10 | 右ペイン（タスク・マーカー）内の絞り込み | B |
| ID-4 | カード一覧のキーボード操作 | B |
| ID-5 | カードの複数選択・一括操作 | B |
| ID-10 | カードのエクスポート（Markdown / CSV） | B |
| CH-11 | 長い会話の日付区切りヘッダー | B |

---

## 4. NestSuite Shell 改善
...
現在の未完了項目なし（TD-1〜TD-33 はすべて完了済み。詳細は `docs/release-notes.md` 参照）。
