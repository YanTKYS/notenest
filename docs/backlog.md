...
| CH-14 | ChatNest 会話の整形コピー | 会話全体または選択範囲を「発言者: 本文」形式でクリップボードへコピーする。NoteNest への貼り付けや議事録化を想定。AI 要約ではなくローカル整形コピーに留める。CH-9（テキスト / Markdown エクスポート）の軽量版として実装でき、既存コピーロジックを再利用しやすい | B |
...
| TD-23 | UI スモークテスト Workspace カバレッジ拡大 | DetachedWindow / TempNest / 各 Workspace 主要 UI の検出拡充。安定性優先。段階的採用候補（v2.11 以降）。注: `TD-18` は v2.8.9 で完了済みのため欠番。本項目は `TD-23` として採番 | C |
...
| SH-20 | 「すべて保存」コマンドの追加 | メニュー「ファイル > すべて保存」（Ctrl+Shift+S）で、未保存の全タブを一括保存する。現在は Ctrl+S でアクティブタブのみ保存。複数タブを編集した際の保存漏れを防ぐ。保存先なしのタブがある場合は SaveAs 経路へ進む。保存失敗・SaveAs キャンセル時は適切に中断する | A |
...
| TD-23 | UI スモークテスト Workspace カバレッジ拡大 | v2.10.1 新規追加（C） |

### 長期構想・保留

以下は将来可能性を残すが現時点では実装しない。詳細は `docs/planning/expert-proposals-2026-06.md` §C を参照。

- 共通データモデルへの全面移行 / `.nestsuite` 統合ファイル形式
- クロス Workspace 双方向リンク / 全文検索
- ナレッジグラフ
- Day / Week Review
- セッション復元の選択的復元
- 複数 Window レイアウト保存
