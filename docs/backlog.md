...
| SH-21 | 複数モニタ向け Workspace 別ウィンドウ表示 | **v2.9.0 で NoteNest を対象とした最小試作を実施。v2.9.1 で安定化完了。** タブ右クリック→「別ウィンドウで表示」で `DetachedWorkspaceWindow` を開く。同 VM 共有・Ctrl+S・ウィンドウ閉じると自動再統合。Shell プレースホルダーの「このタブへ戻す」ボタン・コンテキストメニュー切り替え（未分離: 別ウィンドウで表示 / 分離中: このタブへ戻す）・`IsDetached` / `IsDetachable` フラグを実装。TempNest / IdeaNest / ChatNest は未対応。 | B |
