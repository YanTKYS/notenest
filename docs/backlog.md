...
| SH-17 | タブドラッグ中の挿入位置インジケーター | ドラッグ操作中に、ドロップ先となる位置に細い縦線インジケーターを表示し、タブがどこに挿入されるかを明示する。現行はドロップ後の位置が直感しにくく、意図しない順序になることがある | A |
...
| TN-5 | スロットコピーボタンの完了フィードバック | 「コピー」ボタン押下後にボタン文字を短時間「コピー済」に変化させ、クリップボードへのコピー完了を視覚的に確認できるようにする。SH-13 の共通トーストとは別に、スロット内で完結する軽量フィードバック | A |
| TN-6 | 空スロットのコピー・クリアボタン無効化 | タイトルと本文がともに空のスロットでは「コピー」「クリア」ボタンを無効（`IsEnabled="False"`）にする。現在は空スロットでもボタンが押せ、空文字列がクリップボードに入る | A |
...
| L13 | ノート0件時の空状態ガイドテキスト | 左ペインのノートリストが空のとき「＋ で最初のノートを作成」などのガイドを `MutedFg` 色で表示し、新規プロジェクトを開いたユーザーの操作迷いを防ぐ。`TextBlock` の条件付き表示のみで実現可能 | A |
...
| ID-11 | カード0件・フィルタ結果0件時の空状態テキスト | カードが1件もないとき「＋ でカードを追加」、検索・フィルタ結果が0件のとき「この条件に一致するカードはありません」をグリッド中央に表示する。グリッドが白紙のままにならないようにし、原因をユーザーが把握しやすくする | A |
...
| TD-12 | `NestSuiteTabFactory` / `NestSuiteWorkspaceSessionManager` の単体テスト追加 | `CreateNoteNestTab()` / `CreateTempTab()` のプロパティ検証・Session の追加取得削除ラウンドトリップ・`TryGet` が存在しない ID で `false` を返すことの検証を追加する。TD-10 のスコープ外（TabFactory / SessionManager 直接）を補完する | B |
