...
| ~~TD-32~~ | ~~テストクラス集約~~ | v2.10.18 完了。`SessionNestTD25Tests` / `GuardNestTD26Tests` / `LightImprovementsV2103Tests` を対象クラス単位・責務単位・*Nest 単位の既存受け皿へ集約した。テストクラス数は増やさず 94 → 91 に削減（TD-28 起点の累計では 96 → 91）。テストロジックの意味変更なし。 | — |
