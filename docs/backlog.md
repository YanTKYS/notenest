...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| N4 | NoteNestWorkspaceView 実切り出し | v1.5.4 で確定した移行計画に基づき、`NoteNestWorkspaceView` を実際に作成し `MainWindow` から分離する。DataContext は案 A（`MainViewModel` 継続）で進める | B |
...
※ N4 は v1.5.4 で移行計画（切り出し範囲・手順・回帰確認）を確定済み。v1.5.5 で実切り出しを行う。
