...
- **TD-3: 3 Workspace のファイル操作重複の汎用化**（v2.7.15 完了 → `docs/release-notes.md` 参照）— TD-3-1〜TD-3-4 ですべて完了。
- **TD-5: 軽量ロギング基盤の導入**（v2.7.5 縮小採用完了 → `docs/release-notes.md` 参照）— `ErrorLogService` を Error 専用・追記方式で新設。Info / Warning ログは導入しない方針。
...
| TD-13 | イベント購読の解除漏れ点検・修正 | `+=` によるイベント購読が `86` 箇所に対し `-=` が `32` 箇所のみ。`PreviewIdeaWindow.xaml.cs`（5 += / 2 -=）、`NoteNestWorkspaceView.FilterEvents.cs`（7 += / 6 -=）、`ChatNestWorkspaceView.xaml.cs`（3 += / 2 -=）等のインバランスを点検し、不要な購読残存によるメモリリークを防止する。動作変更なし | B |
| TD-14 | MainViewModel partial の単体テスト追加 | `MainViewModel.Notes.cs` / `MainViewModel.Tasks.cs` / `MainViewModel.Persistence.cs` / `MainViewModel.Links.cs` / `MainViewModel.Markers.cs` / `MainViewModel.Editor.cs` にテストが存在しない。WPF ダイアログ依存を避けられる範囲でロジックの単体テストを整備し、回帰検出力を高める | B |
| TD-15 | NestSuiteShellWindow.Tabs.cs の責務分割 | 579 行。タブ活性化・Workspace 可視性同期・サイドバーハイライト・メニュー更新・ステータスバー更新が混在している。TD-2（partial 分割）の延長として、TabActivationService / WorkspaceSyncService 等へのロジック委譲を検討する。外部シグネチャ変更なし | C |
| TD-16 | NestSuiteShellWindow.FileOperations.cs の責務分割 | 566 行。ファイル保存・読込・エラーハンドリング・パス正規化が混在している。TD-3 で導入した helper 群と合わせ、FileOperationService 等への委譲を検討する。外部シグネチャ変更なし | C |
| TD-17 | ダイアログコンポーネントのテスト追加 | `FindReplaceDialog` / `FontSettingsDialog` / `InputDialog` / `NotePickerDialog` / `BrokenLinksDialog` の 5 ダイアログにテストが存在しない。WPF UI テストは困難だが、ダイアログのロジック部分（検索・入力バリデーション等）を抽出しテスト可能にする。TD-7（`CloseConfirmationService` 方式）を参考にする | B |

---

## 改善提案（v2.7.15 時点のコードベース分析に基づく）

v2.7.15 時点のソースコードを分析し、以下の改善提案を各セクションの既存項目への追記および新規項目として整理した。

### 技術的負債の解消（既存項目への補足）

- **TD-11（AutomationProperties 補完）について:** 現在すべての XAML ファイルで `AutomationProperties` が未設定。ツールランチャー・タブストリップ・カード操作ボタン・検索ボックス・入力欄を優先対象とする。スクリーンリーダー対応だけでなく、将来の UI 自動テスト基盤にもなる。
- **TD-13〜TD-17 は新規追加済み。** イベント購読リーク（TD-13）・MainViewModel テスト（TD-14）・ShellWindow 分割（TD-15/16）・ダイアログテスト（TD-17）。
...
- **SH-10（テーマ切替）について:** IdeaNest は `IdeaNestResources.xaml` に独自カラーパレットをハードコードしており、Shell のテーマ切替と連動しない。テーマ実装時は IdeaNest カラーを `DynamicResource` に移行する前提作業が必要。
...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| SH-19 | ヘルプメニューにキーボードショートカット一覧を追加 | 「ヘルプ > キーボードショートカット」メニューから、Shell 共通（Ctrl+S / Ctrl+Tab 等）と各 Workspace 固有（NoteNest: Ctrl+F、ChatNest: Ctrl+F / Ctrl+Shift+Enter 等、IdeaNest: Ctrl+Shift+N 等）のショートカットをダイアログまたはポップアップで一覧表示する。現状はツールチップでしか確認できない | B |
| SH-20 | 「すべて保存」コマンドの追加 | メニュー「ファイル > すべて保存」（Ctrl+Shift+S）で、未保存の全タブを一括保存する。現在は Ctrl+S でアクティブタブのみ保存。複数タブを編集した際の保存漏れを防ぐ | A |

### NoteNest 機能追加

| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| L15 | ノート本文の文字数・行数表示 | ステータスバーまたはエディタ下部に現在のノートの文字数と行数を表示する。L14（行・列番号表示）と同時に実装可能。TextBox の `Text.Length` と改行数のカウントで実装できる | B |
