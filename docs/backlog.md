...
| No | 項目 | 概要 | 優先度 |
|----|------|------|--------|
| TD-3 | 3 Workspace のファイル操作重複の汎用化 | **第二段階（TD-3-2）完了（v2.7.7）。** Load / Save / SaveAs（TD-3-1）・新規作成・タブクローズ確認（TD-3-2）の共通化済み。残課題: `Sync*TabForViewModel` の共通化、保存成功後の tab/session 更新統一、セッション管理との統合整理 | B |
