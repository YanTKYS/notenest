...
| ファイル | 責務 | 将来の切り出し候補 |
|----------|------|--------------------|
| `MainViewModel.cs` | 画面全体の状態、表示用プロパティ、コマンドの合成 | 子 ViewModel を束ねる shell |
| `MainViewModel.Notes.cs` | ノートブック・ノートの追加、変更、移動、選択、リンク先検索 | `NoteWorkspaceViewModel` |
| `MainViewModel.Tasks.cs` | タスクの追加、変更、移動、コメント編集 | `TaskWorkspaceViewModel` |
| `MainViewModel.Markers.cs` | マーカー抽出結果の更新と絞り込み通知 | `MarkerWorkspaceViewModel` |
