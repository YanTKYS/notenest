...
棚卸し時点で `Markers`、`MarkerCount`、`AllNotes`、`CurrentNoteTitle` はXAML／MainWindowから使用されず、責務所有者側に同等の公開値があるため削除した。ファサード内部から別の単純ファサードを呼ぶ過剰な中継は避け、責務所有者へ直接委譲する。責務所有者の `PropertyChanged` は、MainViewModelが実際に公開するプロパティ名だけを中継する。今後ファサードを追加する場合は、XAML互換・横断調停・UI境界のいずれに必要かを明示する。
