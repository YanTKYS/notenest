...
| v1.8.0 | IdeaNest 統合検証（N20 完了）。IdeaNest v1.1.4 参照ソースから 35 ファイルを `NestSuite/IdeaNest/` へ取り込み。`NestSuiteShellWindow` に `IdeaNestWorkspaceView` を追加しタブ切替で表示。`IdeaNestWorkspaceViewModel`（DirtyRequested イベント・HasChanges・LoadFromWorkspace）を新設。リソースキー競合回避のため全 IdeaNest 固有キーに `Idea` プレフィックス付与。`IsIntegrated=true`（統合検証段階）に変更。`.ideanest` 保存・読込は未対応（情報ダイアログ）。design-decisions.md §42 追加 |
...
| 将来 | IdeaNest `.ideanest` 保存・読込（N21）・複数 NoteNest タブの独立した ViewModel 管理（N17）・MainViewModel Workspace Facade 分離（N6） |
