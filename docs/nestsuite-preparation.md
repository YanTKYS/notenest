...
| v1.8.2 | IdeaNest `.ideanest` 保存・読込方針の整理（N21 準備） |
| 将来 | `.ideanest` 保存・読込の最小対応（N21）・複数 NoteNest タブの独立した ViewModel 管理（N17）・MainViewModel Workspace Facade 分離（N6）・タブ復元・同一ツール複数ファイル対応 |
