...
| v1.8.4 | `.ideanest` 保存／読込と起動時指定の回帰確認・小修正。WorkspaceViewModel の保存・読込・未保存状態テストを追加。NoteNest / ChatNest の保存形式は変更なし。 |
| v1.8.5 | 3ツール統合後の回帰確認・小修正。拡張子の大文字小文字・誤分類防止テストを追加し、NoteNestタブ同期のWorkspaceKind検証を強化。 |
...
| ~~v1.8.2~~ | ~~IdeaNest `.ideanest` 保存・読込方針の整理（N21 準備）~~ → v1.8.2 で完了（モデルに `[JsonPropertyName]` 追加・`IdeaNestFileService` スケルトン・`docs/integration/ideanest-save-load-plan.md` 作成） |
