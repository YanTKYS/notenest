...
> 前提: `docs/design/notenest-editor-host-design.md`（v2.5.4 H0-4）
...
| 順序 | 項目 | 概要 | 前提 |
|------|------|------|------|
| 1 | EH-1: NoteEditorHost 最小実装 | H1 / H2 着手の前提。UI 変更なし | なし |
| 2 | H2a: 行番号ガターの現在行強調 | `LineNumberBox` を ItemsControl に変更し、現在行の行番号を強調 | EH-1 完了後 |
| 3 | H1a: 簡易ノートリンク補完 | `[[` 入力時に候補 Popup を表示し選択で挿入 | EH-1 完了後 |
| — | H3: ノートリンクの視覚的ハイライト | 長期保留。エディタ部品差し替えを決断した場合に再検討 | H3 は保留 |
| — | H4: マーカー行の表示／非表示 | 要望取り下げにより対応しない | — |

EH-1 → H2a → H1a の順を推奨するが、EH-1 → H1a → H2a でも問題ない。H1a と H2a は独立して着手できる。
