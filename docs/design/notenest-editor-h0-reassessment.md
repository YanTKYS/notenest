...
> 前提: `docs/design/notenest-editor-host-design.md`（v2.5.4 H0-4 / v2.5.7 EH-1 実装結果は §8.4）
...
| 2 | H2a: 行番号ガターの現在行強調 | `LineNumberBox` を ItemsControl に変更し、現在行の行番号を強調 | EH-1 完了後 | 未着手 |
...
EH-1 完了により、H2a と H1a はどちらから着手してもよい。H2a → H1a の順を推奨するが、H1a → H2a でも問題ない。
