...
## NestSuite.exe を移動した場合

NestSuite.exe の配置場所を変えた場合は、再度「関連付けを登録」を実行してください。古いパスが登録されたままだと、ファイルをダブルクリックしても起動しません。
...
**登録**:
```powershell
.\tools\register-nestsuite-file-association.ps1 -ExePath "C:\path\to\NestSuite.exe"
```
...
Windows 10 / 11 では、OS の「設定 → アプリ → 既定のアプリ」が `HKCU\Software\Classes` の設定より優先される場合があります。関連付けが反映されない場合は、Windows の設定から手動で NestSuite を既定のアプリとして設定してください。
